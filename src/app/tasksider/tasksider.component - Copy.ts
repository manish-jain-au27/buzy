...
export class TasksiderComponent implements OnInit {
  ...
  pushendHs()
  {
    ...
    this.time = averageTimePerDay;
    this.selesttime = averageTimePerDay;
    const endDateTime=this.dateT+' - '+hourString+':'+this.etm+' '+this.etap;
    this.enddatetimesel = endDateTime;
  }
  counter(i: number) {
    return new Array(i);
  }
  ...
  calculateWorkEndDateTime(workHoursToAdd: any) {
    ...
    const fullWorkDays = Math.floor(workHoursToAdd / workDayHours);
    const remainingHours = workHoursToAdd % workDayHours;

    let endDateTime = momentStart.clone().add(fullWorkDays, 'days');
    const startHour = endDateTime.hours();

    if (startHour + remainingHours <= 17) {
      endDateTime.add(remainingHours, 'hours');
    } else {
      const extraHours = (startHour + remainingHours) - 17;
    ...
    this.selectedTime = `${parseInt(hour, 10)} ${ampmShort}`;
    this.selectedTimeM = minute;
